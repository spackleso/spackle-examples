...
class DynamoDBStore implements Store {
  ...
  private async fetchStateFromApi(customerId: string) {
    console.warn("State not found in DynamoDB, fetching from API...")
...
const fromSpackleCredentials = (spackle: Spackle, session: any, setSession: (s: any) => void): AwsCredentialIdentityProvider => {
  async function spackleCredentials() {
    console.debug("Fetching credentials...")
    if (!session) {
